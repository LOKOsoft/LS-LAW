...
const outstandingConfig = { outstanding: { label: "Outstanding", color: "var(--chart-4)" } } satisfies ChartConfig;
const clientGrowthConfig = { newClients: { label: "New clients", color: "var(--chart-3)" } } satisfies ChartConfig;
const taskRateConfig = { rate: { label: "Completion rate", color: "var(--chart-2)" } } satisfies ChartConfig;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <ChartContainer config={clientGrowthConfig} className="h-56 w-full">
              <AreaChart data={data.clientGrowth} margin={{ left: -12, right: 12, top: 8 }}>
                <defs>
                  <linearGradient id="reportsClientGrowthFill" x1="0" y1="0" x2="0" y2="1">
                ...
                <YAxis tickLine={false} axisLine={false} tickMargin={8} fontSize={12} width={32} allowDecimals={false} />
                <ChartTooltip content={<ChartTooltipContent />} />
                <Area dataKey="newClients" type="monotone" stroke="var(--chart-3)" strokeWidth={2} fill="url(#reportsClientGrowthFill)" dot={false} />
              </AreaChart>
            </ChartContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Top clients by unpaid invoice balance</CardDescription>
          </CardHeader>
          <CardContent>
            {data.outstandingByClient.length === 0 ? (
              <p className="flex h-56 items-center justify-center text-sm text-muted-foreground">No outstanding balances.</p>
            ) : (
              <ChartContainer config={outstandingConfig} className="h-56 w-full">
                <BarChart data={data.outstandingByClient} layout="vertical" margin={{ left: 16 }}>
                  <CartesianGrid horizontal={false} stroke="var(--border)" strokeDasharray="3 3" />
                  <XAxis type="number" tickLine={false} axisLine={false} fontSize={12} tickFormatter={(v: number) => formatCurrencyCompact(v)} />
                  <YAxis type="category" dataKey="name" tickLine={false} axisLine={false} fontSize={12} width={100} />
                  <ChartTooltip content={<ChartTooltipContent formatter={(value) => formatCurrencyCompact(Number(value))} />} />
                  <Bar dataKey="outstanding" fill="var(--chart-4)" radius={[0, 6, 6, 0]} maxBarSize={18} />
                </BarChart>
              </ChartContainer>
            )}
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {data.taskCompletion.completed} of {data.taskCompletion.total} tasks done — {data.taskCompletion.rate}% overall
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ChartContainer config={taskRateConfig} className="h-48 w-full">
            <BarChart data={data.taskCompletion.byPriority} margin={{ top: 8 }}>
              <CartesianGrid vertical={false} stroke="var(--border)" strokeDasharray="3 3" />
              <XAxis dataKey="priority" tickLine={false} axisLine={false} tickMargin={8} fontSize={12} />
              <YAxis tickLine={false} axisLine={false} tickMargin={8} fontSize={12} width={40} tickFormatter={(v: number) => `${v}%`} />
              <ChartTooltip content={<ChartTooltipContent formatter={(value) => formatPercent(Number(value))} />} />
