...
import { ASSOCIATE_BASE, ASSOCIATE_MODULE_KEYS } from "@/lib/constants/nav";
...
export default async function AssociateLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      <Sidebar basePath={ASSOCIATE_BASE} allowedKeys={ASSOCIATE_MODULE_KEYS} roleLabel="Associate" />
      <div className="flex min-w-0 flex-1 flex-col">
        <Topbar
          firmName={firm.name}
          userName={user.name}
          userTitle={user.title ?? "Associate"}
          notifications={notifications}
          unreadCount={unreadCount}
          basePath={ASSOCIATE_BASE}
          allowedKeys={ASSOCIATE_MODULE_KEYS}
        />
