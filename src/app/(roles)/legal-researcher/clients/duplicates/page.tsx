...
export default async function LegalResearcherDuplicateClientsPage() {
  const [groups] = await Promise.all([getDuplicateClientGroups(), requireUser(Role.LEGAL_RESEARCHER)]);
  ...
  return (
    <div>
      ...
      <DuplicateDetectionView groups={groups} basePath={LEGAL_RESEARCHER_BASE} />
    </div>
  );
}
