...
export default async function ParalegalDashboardPage() {
  ...
  return (
    <div className="space-y-8 pb-8">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">
        ...
        <UpcomingDeadlinesCard tasks={data.deadlinesThisWeek} basePath={PARALEGAL_BASE} />
        <MyTimeThisWeekCard {...data.timeThisWeek} />
      </div>

      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <MyMattersCard matters={data.myMatters} basePath={PARALEGAL_BASE} />
        <TaskOverviewCard data={data.taskOverview} description="Your task board status" />
        <MyRecentDocumentsCard documents={data.recentDocuments} basePath={PARALEGAL_BASE} />
      </div>
