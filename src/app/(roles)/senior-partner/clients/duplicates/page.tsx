...
export default async function SeniorPartnerDuplicateClientsPage() {
  const [groups] = await Promise.all([getDuplicateClientGroups(), requireUser(Role.SENIOR_PARTNER)]);
  ...
  return (
    <div>
      ...
      <DuplicateDetectionView groups={groups} basePath={SENIOR_PARTNER_BASE} />
    </div>
  );
}
