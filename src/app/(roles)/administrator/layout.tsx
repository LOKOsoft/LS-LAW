...
import { ALL_MODULE_KEYS, ADMINISTRATOR_BASE } from "@/lib/constants/nav";
...
export default async function AdministratorLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      <Sidebar basePath={ADMINISTRATOR_BASE} allowedKeys={ALL_MODULE_KEYS} roleLabel="Administrator" />
      <div className="flex min-w-0 flex-1 flex-col">
        <Topbar
          firmName={firm.name}
          userName={user.name}
          userTitle={user.title ?? "Administrator"}
